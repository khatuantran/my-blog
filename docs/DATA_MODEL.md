...
- Append-only (chỉ `createdAt`, no `updatedAt`). Ghi best-effort fire-and-forget — KHÔNG block hành động gốc.
- CHỈ ghi khi `actorRole !== ADMIN`. Remove reaction / unlike KHÔNG ghi.
- Soft FK `targetId` → log sống sót khi target bị xoá. `actorUserId` SetNull khi user xoá.
- PII (ip/fingerprint): retention v1 giữ vô hạn, purge thủ công (FR-18.5).
...
### Enum InteractionAction (MỚI — FR-18)

Values: `COMMENT`, `REPLY`, `COMMENT_LIKE`, `POST_REACTION`
Dùng cho: `InteractionLog.action`.

### Enum InteractionTargetType (MỚI — FR-18)

Values: `POST`, `COMMENT`
Dùng cho: `InteractionLog.targetType` (polymorphic soft-FK với `targetId`).
