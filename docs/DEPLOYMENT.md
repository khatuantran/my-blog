...
| `AI_PROVIDER`              | api             | no (only if FR-17 enabled) | `anthropic`                                                   | **NEW M11.8 FR-17** — `anthropic` (default v1) / `openai` / `gemini`                                                                                                                     |
| `AI_API_KEY`               | api             | only if FR-17 enabled      | `sk-ant-...` / `sk-...`                                       | **NEW M11.8 FR-17** — Provider API key. Anthropic: `sk-ant-api...`. OpenAI: `sk-...`. KHÔNG commit. Empty → endpoint trả 500 `AI_NOT_CONFIGURED`                                         |
| `AI_MODEL`                 | api             | no (provider default)      | `claude-haiku-4-5`                                            | **NEW M11.8 FR-17** — Default `claude-haiku-4-5` (cheap, fast cho personal blog). Override nếu cần `claude-sonnet-4-6` higher quality. OpenAI: `gpt-4o-mini`. Gemini: `gemini-1.5-flash` |
| `AI_RATE_LIMIT_PER_MIN`    | api             | no (default 10)            | `10`                                                          | **NEW M11.8 FR-17** — Per-admin rate limit                                                                                                                                               |
