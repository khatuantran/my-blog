...
- **Variants:**
  ...
  - **Engine (T-435, ADR-009):** RichTextEditor dùng **TipTap (ProseMirror)** thay `document.execCommand` (deprecated) → output HTML semantic gọn (`<p>/<strong>/<em>/<u>/<s>/<mark>/<h1>/<h2>/<ul>/<ol>/<a>`), không phình inline-style. Toolbar/popover UI + data-testid giữ nguyên (chỉ đổi cơ chế thực thi → TipTap command). Editor box: `min-height:220px` + `overflow-y-auto` (KHÔNG `resize-y`). Prose styling (h1/h2/ul/ol/mark/a/strong/p margin) ở `globals.css` dùng CHUNG cho editor (`[data-testid='rte-editor']`) + post hiển thị (`[data-testid='post-content-html']`) theo design-file `.editor-area` L29-36. Placeholder qua TipTap Placeholder extension (`p.is-editor-empty:first-child::before`). Counter: text length (`getText`) + cảnh báo khi HTML length ≥ 45000 (BE `@MaxLength(50000)` đếm HTML).
