...
- **Done (T-406 — Profile tabs + buttons refine):** User feedback "filter nhỏ hơn design + button khác". Tabs Saved/Activity bỏ `hidden` → 4 tab luôn visible (content vẫn gate canViewSaved). TabButtons bigger: 11→13px + py-2.5 + active cyan shadow glow; count badge bordered cyan-tint khi active. New Post solid cyan filled glow 14px; Settings đồng kích cỡ. Test stale-assumption: non-self viewer tabs Saved/Activity visible. 67/67 affected pass.
