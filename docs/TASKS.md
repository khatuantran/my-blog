...
- [T-380] [P0] [F3] [FE] Fix BUG-006 AdminPage `/admin` TypeError crash — `Cannot read properties of undefined (reading 'total')` tại `apps/web/src/pages/AdminPage.tsx` L62-83 (4 StatCards destructure `stats.{posts,likes,comments,views}.total`). Investigate root cause (H1 BE response shape drift vs `AdminStatsResponse` interface `apps/web/src/services/api/admin.ts` L20-23 / H2 hook race với partial data / H3 auth error envelope) qua curl `GET /admin/stats` với admin JWT. Fix: defensive optional chaining `stats?.posts?.total ?? 0` cho 4 StatCards + tighten loading guard sang `statsQ.isSuccess` thay vì truthy check. Plus regression test `apps/web/tests/pages/AdminPage.test.tsx` cover 4 case (loading state → AsciiSpinner / success → 4 StatCards render / partial data shape → graceful fallback 0 / error state). Nếu phát hiện BE drift → log thêm BUG hoặc F1 amend `AdminStatsResponse` type. Blocking `/admin` entire page. (BUG-006, FR-07) - TODO
