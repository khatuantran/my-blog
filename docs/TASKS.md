...
- [T-378] [P3] [F3] [FE] Fix BUG-004 ProfilePage ADMIN role badge vertical alignment. `ProfilePage.tsx` L93-103: replace `px-2 py-0.5` với `inline-flex items-center` + `leading-none` + inline `padding: 1px 6px` + subtle `bg-{ora|red}/[0.06]` tint per `design-file/MyBlog Profile.html L488` spec. Pure CSS — không có regression test (visual styling). (BUG-004, FR-11.1) - DONE 2026-05-25
